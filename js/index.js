...
const getTokens = (accountData) => {
	fetch(`https://eosauthority.com/api/spa/account/${accountData.account_name}/tokens?network=wax`, {
  	...
  	return res.json()
  		...
  		let swapPriceData = {"json":true,"code":"alcorammswap","scope":"alcorammswap","table":"pairs","lower_bound":"","upper_bound":"","index_position":1,"key_type":"","limit":1000,"reverse":false,"show_payer":false}
      fetch("https://wax.greymass.com/v1/chain/get_table_rows", {
        ...
        tokenData.forEach((token) => {
        	...
        	let convData = data.rows.find(row => row.pool2.quantity.includes(token.symbol))
          let waxQuantity = parseFloat(convData.pool1.quantity.split(" ")[0])
          let tokenQuantity = parseFloat(convData.pool2.quantity.split(" ")[0])
